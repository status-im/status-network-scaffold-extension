...
if (args.includes("--help") || args.includes("-h")) {
  console.log(`
Usage: yarn status:deploy [options]
Options:
...
Examples:
  yarn status:deploy --file DeployHelloStatusNetwork.s.sol --network statusSepolia
  yarn status:deploy --network statusSepolia --keystore my-account
  yarn status:deploy --file DeployHelloStatusNetwork.s.sol
  yarn status:deploy
  `);
  process.exit(0);
}
...
const result = spawnSync("make", ["deploy-status-and-generate-abis"], {
  stdio: "inherit",
  shell: true,
});
